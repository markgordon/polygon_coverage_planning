...
bool GraphBase<NodeProperty, EdgeProperty>::addStartNode(
  ...
  if (addNode(node_property)) {
    ...
    printf("Failed adding start node.");
    return false;
  }
}
...
bool GraphBase<NodeProperty, EdgeProperty>::addGoalNode(
  ...
  if (addNode(node_property)) {
    ...
    printf("Failed adding goal node.");
    return false;
  }
}
...
bool GraphBase<NodeProperty, EdgeProperty>::calculateHeuristic(
  ...
  printf("Heuristic not implemented.");
  return false;
}
...
GraphBase<NodeProperty, EdgeProperty>::getAdjacencyMatrix() const {
  ...
  if (sorted_cost.back() == 0.0) {
    printf("Adjacency matrix invalid. Greatest cost is 0.0.");
  ...
  if (max_scale * total_cost_upper_bound < std::numeric_limits<int>::max()) {
    printf("Optimal scale %.9f applied.", max_scale);
    scale = max_scale;
  } else {
    printf("Minimum scale %.9f applied.", min_scale);
    scale = min_scale;
    if (min_diff < 1.0) {
      printf("The adjacency matrix is ill conditioned. Consider removing small "
        "details in the polygon to have even decomposition sizes. Loss of "
        "optimality!");
    }
  }

  printf("The minimum cost difference is: %.9f", min_diff);
  printf("The adjacency matrix scale is: %.9f", scale);
