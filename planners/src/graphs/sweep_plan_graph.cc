...
bool SweepPlanGraph::create() {
  ...
  if (!computeDecomposition()) {
    printf("Failed to compute decomposition.");
    return false;
  }
  if (!offsetDecomposition()) {
    printf("Failed to offset neighboring decomposition cells.");
    return false;
  }
  ...
  for (size_t cluster = 0; cluster < polygon_clusters_.size(); ++cluster) {
    ...
    if (settings_.sweep_single_direction) {
      ...
      if (!computeSweep(polygon_clusters_[cluster], vis_graph,
        ...
        std::cout << "Cannot compute single sweep for cluster: " << cluster << std::endl;
        return false;
      }
    } else {
      assert(settings_.sensor_model);
      if (!computeAllSweeps(polygon_clusters_[cluster],
                            ...
                            &cluster_sweeps)) {
        std::cout << "Cannot create all sweep plans for cluster " << cluster << std::endl;
        return false;
      }
    }
  ...
  std::cout << "Created sweep plan graph with "
                  << graph_.size() << " nodes and " << edge_properties_.size()
                  << " edges." << std::endl;
  std::cout << "Initially created " << num_sweep_plans << " nodes." << std::endl;
  std::cout << "Pruned " << num_sweep_plans - graph_.size() << " nodes." << std::endl;
  is_created_ = true;
  return true;
}
  ...
  switch (settings_.decomposition_type) {
    case DecompositionType::kBCD: {
      if (!computeBestBCDFromPolygonWithHoles(settings_.polygon,
                                              &polygon_clusters_)) {
        printf("Cannot compute boustrophedon decomposition.");
        return false;
      } else {
        std::cout << "Successfully created boustrophedon decomposition with "
                        << polygon_clusters_.size() << " polygon(s)." << std::endl;
      }
      break;
    }
    case DecompositionType::kTCD: {
      if (!computeBestTCDFromPolygonWithHoles(settings_.polygon,
                                              &polygon_clusters_)) {
        printf("Cannot compute trapezoidal decomposition.");
        return false;
      } else {
        std::cout << "Successfully created trapezoidal decomposition with "
                        << polygon_clusters_.size() << " polygon(s)." << std::endl;
      }
      break;
    }
    default: {
      printf("No valid decomposition type set.");
      return false;
      break;
    }
  }
...
bool SweepPlanGraph::offsetDecomposition() {
  ...
  if (settings_.offset_polygons && !calculateDecompositionAdjacency(&adj)) {
    printf("Decomposition not fully connected.");
    return false;
  }
  timer_poly_adj.Stop();
  ...
  if (settings_.offset_polygons && !offsetAdjacentCells(adj)) {
    printf("Failed to offset rectangular decomposition.");
    return false;
  }
...
bool SweepPlanGraph::calculateDecompositionAdjacency(
  ...
  assert(decomposition_adjacency);
          ...
          if (result) {
            if (const Segment_2* s = boost::get<Segment_2>(&*result)) {
              if (*s == cell.edge(cell_e) ||
                ...
                printf("Segment intersection but not identical.");
              }
            } else {
              std::cout << "Only point intersection... "
                               << *boost::get<Point_2>(&*result) << std::endl;
            }
          }
        }
      }
...
bool SweepPlanGraph::createNodeProperty(size_t cluster,
  ...
  if (!computeStartAndGoalVisibility(settings_.polygon, waypoints,
                                     &visibility_polygons)) {
    printf("Cannot compute start and goal visibility graph.");
    return false;
  }
...
bool SweepPlanGraph::addEdges() {
  if (graph_.empty()) {
    printf("Cannot add edges to an empty graph.");
    return false;
  }
...
bool SweepPlanGraph::computeEdge(const EdgeId& edge_id,
  ...
  if (from_node_property->waypoints.empty() ||
    ...
    printf("Waypoints in node property are empty.");
    return false;
  }
  ...
  if (!visibility_graph_.solve(from_node_property->waypoints.back(),
    ...
    std::cout << "Cannot compute shortest path from "
                     ...
                     << to_node_property->waypoints.front() << std::endl;
    return false;
  }
...
bool SweepPlanGraph::solve(const Point_2& start, const Point_2& goal,
  ...
  if (!temp_gtsp_graph.addStartNode(start_node) ||
    ...
    printf("Cannot add start and goal.");
    return false;
  }
  ...
  printf("Start solving GTSP");
  if (!solver.solve()) {
    printf("GkMa solution failed.");
    return false;
  }
  printf("Finished solving GTSP");
