...
bool GtsppProductGraph::create() {
  if (sweep_plan_graph_ == nullptr || boolean_lattice_ == nullptr) {
    printf("Sweep plan graph or boolean lattice not set.");
    return false;
  }
  ...
  std::cout << "Created GTSPP product graph with "
...
bool GtsppProductGraph::createOnline() {
  ...
  std::cout << "Created GTSPP product graph with "
                  << graph_.size() << " nodes without edges." << std::endl;
  is_created_ = true;
  return true;
}
...
bool GtsppProductGraph::getWaypoints(const Solution& solution,
  ...
  if (sweep_plan_graph_ == nullptr) {
    printf("Sweep plan graph not set.");
    return false;
  }
