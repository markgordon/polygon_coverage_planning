...
bool computeVisibilityPolygon(const PolygonWithHoles& pwh,
  ...
  if (!pointInPolygon(pwh, query_point)) {
    printf("Query point outside of polygon.");
  }
  if (!isStrictlySimple(pwh)) {
    printf("Polygon is not strictly simple.");
  }
  ...
  if (poly.number_of_unbounded_faces() != 1) {
    printf("Polygon has unbounded curves.");
  }
                 
  if (poly.number_of_faces() != 2) {
    printf("More than one bounded face in polygon.");
  }
  ...
  if ((f = boost::get<VisibilityArrangement::Face_const_handle>(&pl_result))) {
    ...
    while ((he->target()->point() != (*v)->point()) ||
      ...
      if (he == poly.halfedges_end()) {
        printf("Cannot find halfedge corresponding to vertex.");
        return false;
      }
    }
    ...
    printf("Cannot locate query point on arrangement.");
    return false;
  }

  // Result assertion.
  if (fh->is_fictitious()) {
    printf("Visibility polygon is fictitious.");
    return false;
  }
  if (fh->is_unbounded()) {
    printf("Visibility polygon is unbounded.");
    return false;
  }
