...
bool offsetEdge(const Polygon_2& poly, const size_t& edge_id, double offset,
  ...
  *offset_polygon = poly;
  ...
  if (0.5 * polygon.bbox().ymax() <= max_y) {
    ...
    printf("Offset too large. Re-adjusting.");
  }
  ...
  if (intersection_list.size() != 1) {
    printf("Not exactly one resulting intersections.");
    std::cout << "Polygon: " << polygon << std::endl;
    std::cout << "Mask: " << mask << std::endl;
    // printf("Intersections:");
    // for (auto p : intersection_list) printf(p << "\n");
    return false;
  }
  if (intersection_list[0].number_of_holes() > 0) {
    printf("Mask intersection has holes.");
    return false;
  }
  ...
  if (diff_list[0].number_of_holes() > 0) {
    printf("Polygon difference has holes.");
    return false;
  }
...
bool offsetEdgeWithRadialOffset(const Polygon_2& poly, const size_t& edge_id,
  ...
  assert(eq_2(e->source(), e_prev->target()));
  assert(eq_2(e->target(), e_next->source()));
